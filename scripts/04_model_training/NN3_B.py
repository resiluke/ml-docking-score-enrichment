...
# ============================================================
# NN3B_topDS_enrichment_5fold.py
# ============================================================
# This script trains the NN3B Top-DS enrichment model using
...
#
# Based on the average ensemble prediction, TOP_N compounds with
# the lowest predicted docking scores are selected.
#
# The selected subset is divided into five folds:
#   top_80_XX_train.xyz
#   top_20_XX_test.xyz
#
# For each fold:
#   Training set  = in vivo training + TOP_N training subset
#   Validation set = in vivo validation + TOP_N validation subset
#
# The independent final test set:
...
def build_model(input_dim):
    """Create the NN3B neural network architecture."""
...
print(">> NN3B rebuilt version with TOP_N cross-validation")
...
for fold_id, (train_idx, test_idx) in enumerate(kf.split(top_structures), start=1):
    ...
    write(f"top_80_{fold_id:02d}_train.xyz", top_train)
    write(f"top_20_{fold_id:02d}_test.xyz", top_test)

print(">> TOP_N cross-validation files created")


# ============================================================
# 6. TRAIN NN3B USING FOLD-SPECIFIC ENRICHMENT
# ============================================================
...
for fold in range(1, 6):
    ...
    top_train = read(f"top_80_{fold:02d}_train.xyz", index=":")
    top_val = read(f"top_20_{fold:02d}_test.xyz", index=":")

    train_structures = vivo_train + top_train
    val_structures = vivo_val + top_val
    ...
    best_path = f"NN3B_fold{fold}_best.keras"
    final_path = f"NN3B_fold{fold}_final.keras"
    csv_path = f"in_vitro_20_NN3B_fold{fold}.csv"
...
np.save("NN3B_topDS_5fold_results.npy", fold_results)
