...
# ============================================================
# NN4A_interval_enrichment_invitro_5fold.py
# ============================================================
# This script trains the NN4A interval enrichment model using
# only the in vitro dataset.
#
...
#
# The selected interval subset is saved as:
#   interval_pool.xyz
#
# and divided into five cross-validation folds:
#   intervalA_80_XX_train.xyz
#   intervalA_20_XX_test.xyz
#
...
def build_model(input_dim):
    """Create the NN4A neural network architecture."""
...
print(">> NN4A interval enrichment model using in vitro data only")
...
write("interval_pool.xyz", interval_structures)
...
for fold_id, (train_idx, test_idx) in enumerate(kf.split(interval_structures), start=1):
    ...
    write(f"intervalA_80_{fold_id:02d}_train.xyz", interval_train)
    write(f"intervalA_20_{fold_id:02d}_test.xyz", interval_test)

print(">> Interval 5-fold files created")


# ============================================================
# 6. TRAIN NN4A ONLY ON INTERVAL DATA
# ============================================================
...
for fold in range(1, 6):
    ...
    train_structures = read(f"intervalA_80_{fold:02d}_train.xyz", index=":")
    val_structures = read(f"intervalA_20_{fold:02d}_test.xyz", index=":")
    ...
    best_path = f"NN4A_fold{fold}_best.keras"
    final_path = f"NN4A_fold{fold}_final.keras"
    csv_path = f"in_vitro_20_NN4A_fold{fold}.csv"
...
np.save("NN4A_interval_5fold_results.npy", fold_results)
