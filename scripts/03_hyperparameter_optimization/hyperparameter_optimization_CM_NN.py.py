...
# ============================================================
# hyperparameter_optimization_CM_NN.py
# ============================================================
# This script performs hyperparameter optimization of a neural
# network model using Coulomb Matrix molecular descriptors.
#
# The optimization is performed on a subset of the in_vivo.xyz
# dataset. For each trial, Coulomb Matrix descriptors are
# calculated, a neural network is trained, and the resulting MSE
# on the test set is used as the objective function.
#
# The best model is saved as:
#   best_loss_model_CM.keras
#
# The optimization results are exported to:
#   hyperopt_results_CM_NN.csv
# ============================================================
...
BEST_MODEL_FILE = "best_loss_model_CM.keras"
CHECKPOINT_FILE = "best_model_CM.keras"

X_TEST_FILE = "x_test_CM.npy"
Y_TEST_FILE = "y_test_CM.npy"

RESULTS_CSV = "hyperopt_results_CM_NN.csv"
PLOT_FILE = "hyperopt_best_model_CM_prediction.png"
...
max_atoms = max(len(structure) for structure in structures)

print(f">> Number of molecules: {len(structures)}")
print(f">> Maximum number of atoms: {max_atoms}")
...
cm_descriptor = CoulombMatrix(
    n_atoms_max=max_atoms,
    permutation="sorted_l2"
)


def create_cm_descriptors(structures_subset):
    """Create flattened Coulomb Matrix descriptors."""

    descriptors = cm_descriptor.create(
        structures_subset,
        n_jobs=os.cpu_count()
    )
...
def objective(params):
    ...
    try:
        ...
        x_train = create_cm_descriptors(structures_train)
        x_val = create_cm_descriptors(structures_val)
        x_test = create_cm_descriptors(structures_test)

        descriptor_time = time.time() - start_descriptor

        print(f">> CM descriptor calculation time: {descriptor_time:.2f} s")
